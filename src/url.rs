...
pub fn validate_dapi_base_url(base_url: &str, requires_osu_login: bool) -> Result<()> {
    ...
    } else if base_url.matches("{beatmapset_id}").count() == 1 && base_url.matches("{username}").count() == 1 && base_url.matches("{hashed_password}").count() == 1 {
        if !requires_osu_login {
            return Err(format!("Invalid base_url: {}. {}", base_url,url_rule).into());
        }
    } else {
...
pub fn form_url_requires_osu_login(base_url:&str ,beatmapset_id: &u32,username: &str,hashed_password: &str) -> Result<String> {
...
pub fn form_url(base_url:&str,beatmapset_id: &u32,username: &str,hashed_password: &str) -> Result<String> {
...
pub fn form_sayo_api_url(beatmapset_id: &u32) -> String {
    format!("https://txy1.sayobot.cn/beatmaps/download/full/{}?server=auto", beatmapset_id)
}